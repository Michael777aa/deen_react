...
export default function DhikrScreen() {
  ...
  const [savedCounts, setSavedCounts] = useState<{date: string, count: number, dhikr: string}[]>([]);
      ...
      <View style={[styles.container, { backgroundColor: colors[theme].background }]}>
        <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Card style={styles.streakCard}>
              <View style={styles.streakContent}>
                <View style={styles.streakInfo}>
                  <Text style={[styles.streakTitle, { color: colors[theme].text }]}>
                    Your Dhikr Streak
                  </Text>
                  <Text style={[styles.streakSubtitle, { color: colors[theme].inactive }]}>
                    Keep it going!
                  </Text>
                </View>
                <View style={[styles.streakBadge, { backgroundColor: colors[theme].primary }]}>
          ...
          <Card style={styles.dhikrCard}>
            ...
            <Text style={[styles.dhikrTransliteration, { color: colors[theme].text }]}>
              {selectedDhikr.transliteration}
            </Text>
            <Text style={[styles.dhikrTranslation, { color: colors[theme].inactive }]}>
          ...
          <View style={styles.counterContainer}>
            ...
            >
              <View style={[
                ...
                { 
                  ...
                  borderColor: count >= targetCount ? colors[theme].primary : colors[theme].border
                }
              ]}>
                {/* Progress Circle */}
                ...
                >
                  <Text style={[
                    styles.countText, 
                    { 
                      color: count >= targetCount ? colors[theme].primary : colors[theme].text,
                ...
                <Text style={[styles.targetText, { color: colors[theme].inactive }]}>
                  Target: {targetCount}
                </Text>
              </View>
            </Animated.View>
            ...
            <View style={styles.counterButtons}>
              <TouchableOpacity 
                style={[styles.counterButton, { backgroundColor: colors[theme].card }]}
              ...
              <TouchableOpacity 
                style={[styles.mainCounterButton, { backgroundColor: colors[theme].primary }]}
            ...
            <View style={styles.actionButtons}>
              <TouchableOpacity 
                style={[
                  styles.saveButton, 
                  { 
                    backgroundColor: count > 0 ? colors[theme].primary : colors[theme].inactive,
              ...
              <TouchableOpacity 
                style={[styles.shareButton, { backgroundColor: colors[theme].card }]}
              >
                <Share2 size={20} color={colors[theme].text} />
                <Text style={[styles.shareButtonText, { color: colors[theme].text }]}>
                  Share
                </Text>
              </TouchableOpacity>
            </View>
              ...
              >
                <Text 
                  style={[
                    ...
                    { 
                      color: selectedDhikr.id === dhikr.id 
                        ? '#FFFFFF' 
                        : colors[theme].text 
                    }
                  ]}
            ...
            {[33, 99, 100, 500].map((target) => (
              <TouchableOpacity 
                ...
                style={[
                  styles.targetOption,
                  { 
                    backgroundColor: targetCount === target 
                      ? colors[theme].primary 
                      : colors[theme].card
                  }
                ]}
          ...
          <View style={styles.historyHeader}>
            <Text style={[styles.sectionTitle, { color: colors[theme].text, marginBottom: 0 }]}>
              History
            </Text>
            <TouchableOpacity onPress={() => setShowHistory(!showHistory)}>
              <Text style={[styles.toggleHistoryText, { color: colors[theme].primary }]}>
          ...
          {savedCounts.length > 0 ? (
            <>
              {(showHistory ? savedCounts : savedCounts.slice(0, 3)).map((saved, index) => (
                <Card key={index} style={styles.historyCard}>
                  <View style={styles.historyItem}>
                    <View style={styles.historyLeft}>
                      <View style={[styles.historyCountBadge, { backgroundColor: colors[theme].primary + '20' }]}>
                        <Text style={[styles.historyCount, { color: colors[theme].primary }]}>
                          {saved.count}
                        </Text>
                      </View>
                      <View style={styles.historyInfo}>
                        <Text style={[styles.historyDhikr, { color: colors[theme].text }]}>
                          {saved.dhikr}
                        </Text>
                        <Text style={[styles.historyDate, { color: colors[theme].inactive }]}>
              ...
              {!showHistory && savedCounts.length > 3 && (
                <TouchableOpacity 
                  style={[styles.viewAllButton, { borderColor: colors[theme].border }]}
